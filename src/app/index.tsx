...
const Converter: FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                await axios.get('https://api.exchangeratesapi.io/latest', { cancelToken: cancelToken.token }).then((res: any) => {
                    if (res.status === 200) {
                        setRates((rates) => {
    ...
    const optionsTo = useMemo(
        () =>
            Object.keys(rates).map((key: string, index: number) => {
                return (
                    <option key={index} value={key} disabled={key === controlFrom}>
                        {key} - {countries[key]}
                    </option>
                )
            }),
    ...
    const optionsFrom = useMemo(
        () =>
            Object.keys(rates).map((key: string, index: number) => {
                return (
                    <option key={index} value={key} disabled={key === controlTo}>
                        {key} - {countries[key]}
                    </option>
                )
            }),
